	...
	public DSSDocument extendDocument(final DSSDocument toExtendDocument, final ASiCSignatureParameters parameters) throws DSSException {
		try {
			...
			do {
				...
				updatedDocuments.put(signature.getName(), specificService.extendDocument(signature, xadesParameters));updatedDocuments.put(signature.getName(), specificService.extendDocument(signature, xadesParameters));
			...
			while ((entry = getNextZipEntry(zipInputStream)) != null) {
				...
				DSSDocument updatedDocument = updatedDocuments.get(name);
				if (ASiCContainerValidator.isMimetype(name)) {
					...
					final InputStream inputStream = updatedDocument.openStream();

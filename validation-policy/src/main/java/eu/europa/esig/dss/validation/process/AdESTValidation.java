...
public class AdESTValidation {
	...
	private Conclusion process() {
		...
		*/
		...
		for (final XmlDom timestamp : timestamps) {
			...
			if (!checkRevocationAndTimestampDeltaConstraint(signatureConclusion, productionTime)) {
				return signatureConclusion;
			}
  ...
  private boolean checkRevocationAndTimestampDeltaConstraint(Conclusion conclusion, Date productionTime) {
    ...
    if (deltaInMilliseconds.equals(Long.MAX_VALUE)) {
      return true;
    }
    Long delta = calculateRevocationAndTimeStampDelta(productionTime);
  ...
  private Long calculateRevocationAndTimeStampDelta(Date productionTime) {
    final List<XmlDom> certificateChainXmlDom = signatureXmlDom.getElements(
        "./CertificateChain/ChainCertificate//Source[text()=\"SIGNATURE\"]/..");

    if (certificateChainXmlDom.size() == 0) {
      return null;
    }

    final String certificateId = certificateChainXmlDom.get(0).getValue("./@Id");
    final XmlDom certificateXmlDom = certPool.getElement("./Certificate[@Id='%s']", certificateId);

    final XmlDom revocation = certificateXmlDom.getElement("./Revocation");
    Date producedAtTime = revocation.getTimeValue("./IssuingTime/text()");

    return Math.abs(producedAtTime.getTime() - productionTime.getTime());
  }

...
public class SignatureCryptographicConstraint extends Constraint {
    ...
    public boolean check() {
        ...
        int keyLengthInt = "?".equals(keyLength) ? 0 : Integer.valueOf(keyLength);
        ...
        if (!algorithmExpirationDates.isEmpty()) {
            ...
            boolean expiredAlgorithm = algorithmExpirationDate.before(currentTime);
            if (expiredAlgorithm) {
            ...
            expiredAlgorithm = algorithmExpirationDate.before(currentTime);
            if (expiredAlgorithm) {
    ...
    private Pair[] getParametersAnswer5(final String algorithm, final Date algorithmExpirationDate) {
        ...
        pairs[2] = new Pair(ALGORITHM_EXPIRATION_DATE, RuleUtils.formatDate(algorithmExpirationDate));

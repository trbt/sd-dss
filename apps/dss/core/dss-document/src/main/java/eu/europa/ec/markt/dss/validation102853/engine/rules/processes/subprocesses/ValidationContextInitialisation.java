...
public class ValidationContextInitialisation implements RuleConstant, Indication, SubIndication, NodeName, NodeValue, AttributeName, AttributeValue, ExceptionMessage {
    ...
    private boolean process(final ProcessParameters params, final XmlNode conclusionNode) {
        ...
        if (policyId.isEmpty()) {
            ...
            if (!constraintData.isNoPolicyAcceptable()) {
            ...
            if (constraintData.isAnyPolicyAcceptable()) {
                ...
                final boolean policyValidity = signatureContext.getBoolValue("./Policy/Status/text()");
                if (!policyValidity) {

                    addProcessingErrorPolicyNode(constraintNode, conclusionNode);
                    return false;
                }
                if (!constraintData.isAnyPolicyAcceptable()) {
    ...
    private void addPolicyNode(final XmlNode constraintNode, final String policy) {
        ...
        final String notice = signatureContext.getValue("./Policy/Notice/text()");

        constraintNode.addChild(STATUS, OK);
        constraintNode.addChild(INFO, policy).setAttribute(FIELD, POLICY);
        if (!notice.isEmpty()) {

            constraintNode.addChild(NOTICE, notice);
        }
    }
    ...
    private void addNoPolicyNode(final XmlNode constraintNode, final XmlNode conclusionNode) {
        ...
        conclusionNode.addChild(SUB_INDICATION, SubIndication.NO_POLICY);
    }

    /**
     * @param constraintNode
     */
    private void addProcessingErrorPolicyNode(final XmlNode constraintNode, final XmlNode conclusionNode) {

        final String processingError = signatureContext.getValue("./Policy/ProcessingError/text()");

        constraintNode.addChild(STATUS, KO);
        conclusionNode.addChild(INDICATION, INDETERMINATE);
        conclusionNode.addChild(SUB_INDICATION, POLICY_PROCESSING_ERROR);
        conclusionNode.addChild(INFO, processingError);
    }
